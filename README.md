...
- 文本替换是串行的全文替换，比如：`123456`，替换文本是`123->222`和`24->34`，那么会先替换成`222456`，接着被替换成`223456`。多文本替换时注意顺序和关系。
...
自动翻译的一些选项在`更多设置`里可修改，会有一定的误触发，而且使用自动翻译，容易频繁的调用接口，消耗掉免费的接口次数，看情况选择是否开启。
...
某些设置是不会有UI界面进行配置，如果需要更多的自定义，则要把BKTrans关闭后，手动编辑`setting.json`，更改保存后重启生效。下面列出了可供修改的配置，其余的如果不清楚作用请不要动。

```json
{
  // ...
  // 自动翻译扫描截图区域是否发生变化的周期，ms
  ...
  // 自动翻译截图区域相似度，
  // 低于这个值则认为发生了改变，重置countdown
  // 当高于这个值则认为改变稳定，开始截图翻译
  "auto_captrue_trans_similarity": 0.95,
